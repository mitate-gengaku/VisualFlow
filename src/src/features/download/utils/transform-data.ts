...
import { ResultNode } from "../types/result-node";
...
export class TransformDataClass {
  ...
  transformData(edges: Edge[]): ResultNode {
    ...
    // 全てのノードとその親子関係を記録
    const nodeRelations: { [key: string]: { parents: Set<string>, children: Set<string> } } = {};
  
    edges.forEach(edge => {
      if (!nodeRelations[edge.source]) {
        nodeRelations[edge.source] = { parents: new Set(), children: new Set() };
      }
      if (!nodeRelations[edge.target]) {
        nodeRelations[edge.target] = { parents: new Set(), children: new Set() };
      }
    ...
    const rootNodes = Object.keys(nodeRelations).filter(node => nodeRelations[node].parents.size === 0);
  ...
  replaceIdsWithNodes(transformedData: ResultNode, originalNodes: Node[]): TreeNode[] {
